...
.um-hero-stats { display:flex; justify-content:center; gap:40px; flex-wrap:wrap; }
.um-stat {
...
.um-stat:hover { transform:translateY(-3px); border-color:rgba(232,196,106,0.4); }
...
.um-badge { display:inline-flex; align-items:center; gap:6px; background:rgba(232,196,106,0.12); border:1px solid rgba(232,196,106,0.3); color:#e8c46a; font-size:12px; font-weight:600; padding:6px 14px; border-radius:50px; letter-spacing:1px; text-transform:uppercase; margin-bottom:20px; }
...
@media (max-width:768px) {
  ...
  .um-hero-stats { gap:12px; }
  .um-stat { padding:12px 16px; min-width:75px; }
...
function BookingForm({ pkg, onClose }) {
  ...
  const labelStyle = {
    ...
    color: "rgba(26,26,46,0.5)", marginBottom: 6,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div><label style={labelStyle}>Full Name *</label>
        <input placeholder="e.g. Ahmed Khan" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} style={inputStyle}
      ...
      <div><label style={labelStyle}>Email <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>(Optional)</span></label>
        <input type="email" placeholder="example@email.com" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} style={inputStyle}
          onFocus={e => e.target.style.borderColor = theme.accent} onBlur={e => e.target.style.borderColor = "rgba(0,0,0,0.1)"} />
      </div>
      <div><label style={labelStyle}>City *</label>
        <input placeholder="e.g. Lahore, Karachi, Islamabad" value={form.city} onChange={e => setForm({ ...form, city: e.target.value })} style={inputStyle}
          onFocus={e => e.target.style.borderColor = theme.accent} onBlur={e => e.target.style.borderColor = "rgba(0,0,0,0.1)"} />
      </div>
      <div><label style={labelStyle}>Message <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>(Optional)</span></label>
        <textarea rows={3} placeholder="Koi khaas zaroorat ya sawal..." value={form.message} onChange={e => setForm({ ...form, message: e.target.value })}
      ...
      <button onClick={handleSubmit} disabled={loading || !form.name || !form.phone || !form.city}
        style={{
          ...
          color: "#0a0a0f", border: "none", padding: "15px", borderRadius: 50,
          ...
          {!showBooking ? (
            <>
              ...
              <div className="um-section">
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {pkg.hotels.map((hotel, i) => (
                    <div key={i} style={{ background: theme.bgCard, border: `1px solid ${theme.border}`, borderRadius: 14, padding: "16px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 15, color: theme.text, marginBottom: 4 }}>{hotel.name}</div>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                          <Stars count={hotel.stars} />
                          <span style={{ fontSize: 12, color: theme.textMuted }}>📍 {hotel.distanceCity} — {hotel.distance}m from Haram</span>
                        </div>
                      </div>
                      <div style={{ background: "rgba(232,196,106,0.1)", border: "1px solid rgba(232,196,106,0.25)", borderRadius: 10, padding: "6px 14px", textAlign: "center" }}>
                        <div style={{ fontSize: 11, color: theme.textMuted, marginBottom: 2 }}>Category</div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: theme.accent }}>{"⭐".repeat(hotel.stars)} ({hotel.stars}-Star)</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="um-section">
                ...
                <div style={{ background: theme.bgCard, border: `1px solid ${theme.border}`, borderRadius: 14, padding: "18px 20px", display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
                  <div style={{ fontSize: 42 }}>{pkg.flight.logo}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 800, fontSize: 17, color: theme.text, marginBottom: 4 }}>{pkg.flight.airline}</div>
  ...
  return (
    <div className="um-card" onClick={onClick} style={{ background: theme.bgCard, border: `1px solid ${theme.border}` }}>
      ...
      <div className="um-card-body">
        ...
        <button className="um-card-btn" style={{ background: theme.accent, color: "#0a0a0f" }}
